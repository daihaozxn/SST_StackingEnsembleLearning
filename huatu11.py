...
# 包含persistence model 和 weighted average
...
def MeticsSpatialDistributionShow3(lead_time):
    ...
    # persistence RMSE CE
    yhat_test_Persist_tw = raw_test_tw[(n_steps - 1):-lead_time, :]
    yhat_test_Persist_tw = yhat_test_Persist_tw.reshape(len(yhat_test_Persist_tw), -1)
    rmse_test_Persist_Spatial_tw = sqrt(mean((inv_y_test_tw - yhat_test_Persist_tw) ** 2, axis=0))
    ce_test_Persist_Spatial_tw = 1 - (sum((inv_y_test_tw - yhat_test_Persist_tw) ** 2, axis=0)) / (sum((inv_y_test_tw - mean(inv_y_test_tw)) ** 2, axis=0))
    ...
    # weighted average RMSE CE
    # 根据各个0级模型预测结果的rmses计算逆权重，加权平均作为最终的预测结果，然后计算rmse
    rmse_test_MLP_tw = mean_squared_error(inv_y_test_tw, inv_yhat_test_MLP_tw, squared=False)
    rmse_test_LSTM_tw = mean_squared_error(inv_y_test_tw, inv_yhat_test_LSTM_tw, squared=False)
    rmse_test_CNN_tw = mean_squared_error(inv_y_test_tw, inv_yhat_test_CNN_tw, squared=False)
    rmse_test_CNNLSTM_tw = mean_squared_error(inv_y_test_tw, inv_yhat_test_CNNLSTM_tw, squared=False)

    rmse_test_sum_tw = rmse_test_MLP_tw + rmse_test_LSTM_tw + rmse_test_CNN_tw + rmse_test_CNNLSTM_tw
    inv_rmse_test_sum_tw = rmse_test_sum_tw / rmse_test_MLP_tw + rmse_test_sum_tw / rmse_test_LSTM_tw + rmse_test_sum_tw / rmse_test_CNN_tw + rmse_test_sum_tw / rmse_test_CNNLSTM_tw
    weights_test_MLP_tw = (rmse_test_sum_tw / rmse_test_MLP_tw) / inv_rmse_test_sum_tw
    weights_test_LSTM_tw = (rmse_test_sum_tw / rmse_test_LSTM_tw) / inv_rmse_test_sum_tw
    weights_test_CNN_tw = (rmse_test_sum_tw / rmse_test_CNN_tw) / inv_rmse_test_sum_tw
    weights_test_CNNLSTM_tw = (rmse_test_sum_tw / rmse_test_CNNLSTM_tw) / inv_rmse_test_sum_tw
    yhat_test_ModelsWeightAve_tw = weights_test_MLP_tw * yhat_test_MLP_tw + weights_test_LSTM_tw * yhat_test_LSTM_tw + weights_test_CNN_tw * yhat_test_CNN_tw + weights_test_CNNLSTM_tw * yhat_test_CNNLSTM_tw
    inv_yhat_test_ModelsWeightAve_tw = scaler_tw.inverse_transform(yhat_test_ModelsWeightAve_tw)
    rmse_test_ModelsWeightAve_Spatial_tw = sqrt(mean((inv_y_test_tw - inv_yhat_test_ModelsWeightAve_tw) ** 2, axis=0))
    ce_test_ModelsWeightAve_Spatial_tw = 1 - (sum((inv_y_test_tw - inv_yhat_test_ModelsWeightAve_tw) ** 2, axis=0)) / (sum((inv_y_test_tw - mean(inv_y_test_tw)) ** 2, axis=0))
    ...
    vmin_rmse_tw = array([rmse_test_Persist_Spatial_tw, rmse_test_MLP_Spatial_tw, rmse_test_LSTM_Spatial_tw, rmse_test_CNN_Spatial_tw, rmse_test_CNNLSTM_Spatial_tw, \
                          rmse_test_ModelsWeightAve_Spatial_tw, rmse_test_ConvLSTMStacking_Spatial_tw]).min()
    vmax_rmse_tw = array([rmse_test_Persist_Spatial_tw, rmse_test_MLP_Spatial_tw, rmse_test_LSTM_Spatial_tw, rmse_test_CNN_Spatial_tw, rmse_test_CNNLSTM_Spatial_tw, \
                          rmse_test_ModelsWeightAve_Spatial_tw, rmse_test_ConvLSTMStacking_Spatial_tw]).max()
    norm_rmse_tw = colors.Normalize(vmin=vmin_rmse_tw, vmax=vmax_rmse_tw)

    vmin_ce_tw = array([ce_test_Persist_Spatial_tw, ce_test_MLP_Spatial_tw, ce_test_LSTM_Spatial_tw, ce_test_CNN_Spatial_tw, ce_test_CNNLSTM_Spatial_tw, \
                        ce_test_ModelsWeightAve_Spatial_tw, ce_test_ConvLSTMStacking_Spatial_tw]).min()
    vmax_ce_tw = array([ce_test_Persist_Spatial_tw, ce_test_MLP_Spatial_tw, ce_test_LSTM_Spatial_tw, ce_test_CNN_Spatial_tw, ce_test_CNNLSTM_Spatial_tw, \
                        ce_test_ModelsWeightAve_Spatial_tw, ce_test_ConvLSTMStacking_Spatial_tw]).max()
    norm_ce_tw = colors.Normalize(vmin=vmin_ce_tw, vmax=vmax_ce_tw)

    rmse_test_Persist_Spatial_tw = rmse_test_Persist_Spatial_tw.reshape(n_rows, n_cols)
    ...
    rmse_test_ModelsWeightAve_Spatial_tw = rmse_test_ModelsWeightAve_Spatial_tw.reshape(n_rows, n_cols)

    ce_test_Persist_Spatial_tw = ce_test_Persist_Spatial_tw.reshape(n_rows, n_cols)
    ...
    ce_test_ModelsWeightAve_Spatial_tw = ce_test_ModelsWeightAve_Spatial_tw.reshape(n_rows, n_cols)
    ...
    # persistence RMSE CE
    yhat_test_Persist_dh = raw_test_dh[(n_steps - 1):-lead_time, :]
    yhat_test_Persist_dh = yhat_test_Persist_dh.reshape(len(yhat_test_Persist_dh), -1)
    rmse_test_Persist_Spatial_dh = sqrt(mean((inv_y_test_dh - yhat_test_Persist_dh) ** 2, axis=0))
    ce_test_Persist_Spatial_dh = 1 - (sum((inv_y_test_dh - yhat_test_Persist_dh) ** 2, axis=0)) / (sum((inv_y_test_dh - mean(inv_y_test_dh)) ** 2, axis=0))
    ...
    # weighted average RMSE CE
    # 根据各个0级模型预测结果的rmses计算逆权重，加权平均作为最终的预测结果，然后计算rmse
    rmse_test_MLP_dh = mean_squared_error(inv_y_test_dh, inv_yhat_test_MLP_dh, squared=False)
    rmse_test_LSTM_dh = mean_squared_error(inv_y_test_dh, inv_yhat_test_LSTM_dh, squared=False)
    rmse_test_CNN_dh = mean_squared_error(inv_y_test_dh, inv_yhat_test_CNN_dh, squared=False)
    rmse_test_CNNLSTM_dh = mean_squared_error(inv_y_test_dh, inv_yhat_test_CNNLSTM_dh, squared=False)

    rmse_test_sum_dh = rmse_test_MLP_dh + rmse_test_LSTM_dh + rmse_test_CNN_dh + rmse_test_CNNLSTM_dh
    inv_rmse_test_sum_dh = rmse_test_sum_dh / rmse_test_MLP_dh + rmse_test_sum_dh / rmse_test_LSTM_dh + rmse_test_sum_dh / rmse_test_CNN_dh + rmse_test_sum_dh / rmse_test_CNNLSTM_dh
    weights_test_MLP_dh = (rmse_test_sum_dh / rmse_test_MLP_dh) / inv_rmse_test_sum_dh
    weights_test_LSTM_dh = (rmse_test_sum_dh / rmse_test_LSTM_dh) / inv_rmse_test_sum_dh
    weights_test_CNN_dh = (rmse_test_sum_dh / rmse_test_CNN_dh) / inv_rmse_test_sum_dh
    weights_test_CNNLSTM_dh = (rmse_test_sum_dh / rmse_test_CNNLSTM_dh) / inv_rmse_test_sum_dh
    yhat_test_ModelsWeightAve_dh = weights_test_MLP_dh * yhat_test_MLP_dh + weights_test_LSTM_dh * yhat_test_LSTM_dh + weights_test_CNN_dh * yhat_test_CNN_dh + weights_test_CNNLSTM_dh * yhat_test_CNNLSTM_dh
    inv_yhat_test_ModelsWeightAve_dh = scaler_dh.inverse_transform(yhat_test_ModelsWeightAve_dh)
    rmse_test_ModelsWeightAve_Spatial_dh = sqrt(mean((inv_y_test_dh - inv_yhat_test_ModelsWeightAve_dh) ** 2, axis=0))
    ce_test_ModelsWeightAve_Spatial_dh = 1 - (sum((inv_y_test_dh - inv_yhat_test_ModelsWeightAve_dh) ** 2, axis=0)) / (sum((inv_y_test_dh - mean(inv_y_test_dh)) ** 2, axis=0))
    ...
    vmin_rmse_dh = array([rmse_test_Persist_Spatial_dh, rmse_test_MLP_Spatial_dh, rmse_test_LSTM_Spatial_dh, rmse_test_CNN_Spatial_dh, rmse_test_CNNLSTM_Spatial_dh, \
                          rmse_test_ModelsWeightAve_Spatial_dh, rmse_test_ConvLSTMStacking_Spatial_dh]).min()
    vmax_rmse_dh = array([rmse_test_Persist_Spatial_dh, rmse_test_MLP_Spatial_dh, rmse_test_LSTM_Spatial_dh, rmse_test_CNN_Spatial_dh, rmse_test_CNNLSTM_Spatial_dh, \
                          rmse_test_ModelsWeightAve_Spatial_dh, rmse_test_ConvLSTMStacking_Spatial_dh]).max()
    norm_rmse_dh = colors.Normalize(vmin=vmin_rmse_dh, vmax=vmax_rmse_dh)

    vmin_ce_dh = array([ce_test_Persist_Spatial_dh, ce_test_MLP_Spatial_dh, ce_test_LSTM_Spatial_dh, ce_test_CNN_Spatial_dh, ce_test_CNNLSTM_Spatial_dh, \
                        ce_test_ModelsWeightAve_Spatial_dh, ce_test_ConvLSTMStacking_Spatial_dh]).min()
    vmax_ce_dh = array([ce_test_Persist_Spatial_dh, ce_test_MLP_Spatial_dh, ce_test_LSTM_Spatial_dh, ce_test_CNN_Spatial_dh, ce_test_CNNLSTM_Spatial_dh, \
                        ce_test_ModelsWeightAve_Spatial_dh, ce_test_ConvLSTMStacking_Spatial_dh]).max()
    norm_ce_dh = colors.Normalize(vmin=vmin_ce_dh, vmax=vmax_ce_dh)

    rmse_test_Persist_Spatial_dh = rmse_test_Persist_Spatial_dh.reshape(n_rows, n_cols)
    ...
    rmse_test_ModelsWeightAve_Spatial_dh = rmse_test_ModelsWeightAve_Spatial_dh.reshape(n_rows, n_cols)

    ce_test_Persist_Spatial_dh = ce_test_Persist_Spatial_dh.reshape(n_rows, n_cols)
    ...
    ce_test_ModelsWeightAve_Spatial_dh = ce_test_ModelsWeightAve_Spatial_dh.reshape(n_rows, n_cols)

    fig, axs = plt.subplots(nrows=4, ncols=7, figsize=(20, 18))
    #Persistence
    a = axs[0, 0].pcolormesh(rmse_test_Persist_Spatial_tw, norm=norm_rmse_tw, cmap='jet', shading='auto')
    axs[0, 0].xaxis.set_ticks([])
    axs[0, 0].yaxis.set_ticks([])
    axs[0, 0].set_title('Persist', fontsize=16, weight='bold', loc='center')
    axs[0, 0].set_ylabel('RMSE(℃) \n for Taiwan Strait', fontsize=16, weight='bold')
    # MLP
    axs[0, 1].pcolormesh(rmse_test_MLP_Spatial_tw, norm=norm_rmse_tw, cmap='jet', shading='auto')
    axs[0, 1].xaxis.set_ticks([])
    axs[0, 1].yaxis.set_ticks([])
    axs[0, 1].set_title('MLP', fontsize=16, weight='bold', loc='center')
    # axs[0, 1].set_ylabel('RMSE(℃) \n for Taiwan Strait', fontsize=16, weight='bold')
    # LSTM
    axs[0, 2].pcolormesh(rmse_test_LSTM_Spatial_tw, norm=norm_rmse_tw, cmap='jet', shading='auto')
    axs[0, 2].xaxis.set_ticks([])
    axs[0, 2].yaxis.set_ticks([])
    axs[0, 2].set_title('LSTM', fontsize=16, weight='bold', loc='center')
    # axs[0, 1].set_ylabel('RMSE', fontsize=16, weight='bold')
    # CNN
    axs[0, 3].pcolormesh(rmse_test_CNN_Spatial_tw, norm=norm_rmse_tw, cmap='jet', shading='auto')
    axs[0, 3].xaxis.set_ticks([])
    axs[0, 3].yaxis.set_ticks([])
    axs[0, 3].set_title('CNN', fontsize=16, weight='bold', loc='center')
    # CNNLSTM
    axs[0, 4].pcolormesh(rmse_test_CNNLSTM_Spatial_tw, norm=norm_rmse_tw, cmap='jet', shading='auto')
    axs[0, 4].xaxis.set_ticks([])
    axs[0, 4].yaxis.set_ticks([])
    axs[0, 4].set_title('CNNLSTM', fontsize=16, weight='bold', loc='center')
    # Weighted average
    axs[0, 5].pcolormesh(rmse_test_ModelsWeightAve_Spatial_tw, norm=norm_rmse_tw, cmap='jet', shading='auto')
    axs[0, 5].xaxis.set_ticks([])
    axs[0, 5].yaxis.set_ticks([])
    axs[0, 5].set_title('WeighAvg', fontsize=16, weight='bold', loc='center')
    # ConvLSTMStacking
    axs[0, 6].pcolormesh(rmse_test_ConvLSTMStacking_Spatial_tw, norm=norm_rmse_tw, cmap='jet', shading='auto')
    axs[0, 6].xaxis.set_ticks([])
    axs[0, 6].yaxis.set_ticks([])
    axs[0, 6].set_title('Stacking', fontsize=16, weight='bold', loc='center')
    fig.colorbar(a, ax=axs[0, :])

    #Persistence model
    b = axs[1, 0].pcolormesh(rmse_test_Persist_Spatial_dh, norm=norm_rmse_dh, cmap='jet', shading='auto')
    axs[1, 0].xaxis.set_ticks([])
    axs[1, 0].yaxis.set_ticks([])
    axs[1, 0].set_ylabel('RMSE(℃) \n for East China Sea', fontsize=16, weight='bold')
    # MLP
    axs[1, 1].pcolormesh(rmse_test_MLP_Spatial_dh, norm=norm_rmse_dh, cmap='jet', shading='auto')
    axs[1, 1].xaxis.set_ticks([])
    axs[1, 1].yaxis.set_ticks([])
    # axs[0, 0].set_title('MLP', fontsize=16, weight='bold', loc='center')
    # LSTM
    axs[1, 2].pcolormesh(rmse_test_LSTM_Spatial_dh, norm=norm_rmse_dh, cmap='jet', shading='auto')
    axs[1, 2].xaxis.set_ticks([])
    axs[1, 2].yaxis.set_ticks([])
    # axs[1, 1].set_title('LSTM', fontsize=16, weight='bold', loc='center')
    # axs[0, 1].set_ylabel('RMSE', fontsize=16, weight='bold')
    # CNN
    axs[1, 3].pcolormesh(rmse_test_CNN_Spatial_dh, norm=norm_rmse_dh, cmap='jet', shading='auto')
    axs[1, 3].xaxis.set_ticks([])
    axs[1, 3].yaxis.set_ticks([])
    # axs[1, 2].set_title('CNN', fontsize=16, weight='bold', loc='center')
    # CNNLSTM
    axs[1, 4].pcolormesh(rmse_test_CNNLSTM_Spatial_dh, norm=norm_rmse_dh, cmap='jet', shading='auto')
    axs[1, 4].xaxis.set_ticks([])
    axs[1, 4].yaxis.set_ticks([])
    # axs[1, 3].set_title('CNNLSTM', fontsize=16, weight='bold', loc='center')
    # Weighted Average
    axs[1, 5].pcolormesh(rmse_test_ModelsWeightAve_Spatial_dh, norm=norm_rmse_dh, cmap='jet', shading='auto')
    axs[1, 5].xaxis.set_ticks([])
    axs[1, 5].yaxis.set_ticks([])
    # ConvLSTMStacking
    axs[1, 6].pcolormesh(rmse_test_ConvLSTMStacking_Spatial_dh, norm=norm_rmse_dh, cmap='jet', shading='auto')
    axs[1, 6].xaxis.set_ticks([])
    axs[1, 6].yaxis.set_ticks([])
    # axs[1, 4].set_title('Stacking', fontsize=16, weight='bold', loc='center')
    fig.colorbar(b, ax=axs[1, :])

    # Persistence Model
    c = axs[2, 0].pcolormesh(ce_test_Persist_Spatial_tw, norm=norm_ce_tw, cmap='jet', shading='auto')
    axs[2, 0].xaxis.set_ticks([])
    axs[2, 0].yaxis.set_ticks([])
    axs[2, 0].set_ylabel('CE \n for Taiwan Strait', fontsize=16, weight='bold')
    # MLP
    axs[2, 1].pcolormesh(ce_test_MLP_Spatial_tw, norm=norm_ce_tw, cmap='jet', shading='auto')
    axs[2, 1].xaxis.set_ticks([])
    axs[2, 1].yaxis.set_ticks([])
    # axs[1, 0].set_title('MLP', fontsize=16, weight='bold', loc='center')
    # LSTM
    axs[2, 2].pcolormesh(ce_test_LSTM_Spatial_tw, norm=norm_ce_tw, cmap='jet', shading='auto')
    axs[2, 2].xaxis.set_ticks([])
    axs[2, 2].yaxis.set_ticks([])
    # axs[1, 0].set_title('MLP', fontsize=16, weight='bold', loc='center')
    # axs[1, 1].set_ylabel('CE', fontsize=16, weight='bold')
    # CNN
    axs[2, 3].pcolormesh(ce_test_CNN_Spatial_tw, norm=norm_ce_tw, cmap='jet', shading='auto')
    axs[2, 3].xaxis.set_ticks([])
    axs[2, 3].yaxis.set_ticks([])
    # CNNLSTM
    axs[2, 4].pcolormesh(ce_test_CNNLSTM_Spatial_tw, norm=norm_ce_tw, cmap='jet', shading='auto')
    axs[2, 4].xaxis.set_ticks([])
    axs[2, 4].yaxis.set_ticks([])
    # Weighted Average
    axs[2, 5].pcolormesh(ce_test_ModelsWeightAve_Spatial_tw, norm=norm_ce_tw, cmap='jet', shading='auto')
    axs[2, 5].xaxis.set_ticks([])
    axs[2, 5].yaxis.set_ticks([])
    # ConvLSTMStacking
    axs[2, 6].pcolormesh(ce_test_ConvLSTMStacking_Spatial_tw, norm=norm_ce_tw, cmap='jet', shading='auto')
    axs[2, 6].xaxis.set_ticks([])
    axs[2, 6].yaxis.set_ticks([])
    fig.colorbar(c, ax=axs[2, :])

    # Persistence Model
    d = axs[3, 0].pcolormesh(ce_test_Persist_Spatial_dh, norm=norm_ce_dh, cmap='jet', shading='auto')
    axs[3, 0].xaxis.set_ticks([])
    axs[3, 0].yaxis.set_ticks([])
    axs[3, 0].set_ylabel('CE \n for East China Sea', fontsize=16, weight='bold')
    # MLP
    axs[3, 1].pcolormesh(ce_test_MLP_Spatial_dh, norm=norm_ce_dh, cmap='jet', shading='auto')
    axs[3, 1].xaxis.set_ticks([])
    axs[3, 1].yaxis.set_ticks([])
    # axs[1, 0].set_title('MLP', fontsize=16, weight='bold', loc='center')
    # LSTM
    axs[3, 2].pcolormesh(ce_test_LSTM_Spatial_dh, norm=norm_ce_dh, cmap='jet', shading='auto')
    axs[3, 2].xaxis.set_ticks([])
    axs[3, 2].yaxis.set_ticks([])
    # axs[1, 0].set_title('MLP', fontsize=16, weight='bold', loc='center')
    # axs[1, 1].set_ylabel('CE', fontsize=16, weight='bold')
    # CNN
    axs[3, 3].pcolormesh(ce_test_CNN_Spatial_dh, norm=norm_ce_dh, cmap='jet', shading='auto')
    axs[3, 3].xaxis.set_ticks([])
    axs[3, 3].yaxis.set_ticks([])
    # CNNLSTM
    axs[3, 4].pcolormesh(ce_test_CNNLSTM_Spatial_dh, norm=norm_ce_dh, cmap='jet', shading='auto')
    axs[3, 4].xaxis.set_ticks([])
    axs[3, 4].yaxis.set_ticks([])
    # Weighted Average
    axs[3, 5].pcolormesh(ce_test_ModelsWeightAve_Spatial_dh, norm=norm_ce_dh, cmap='jet', shading='auto')
    axs[3, 5].xaxis.set_ticks([])
    axs[3, 5].yaxis.set_ticks([])
    # ConvLSTMStacking
    axs[3, 6].pcolormesh(ce_test_ConvLSTMStacking_Spatial_dh, norm=norm_ce_dh, cmap='jet', shading='auto')
    axs[3, 6].xaxis.set_ticks([])
    axs[3, 6].yaxis.set_ticks([])
...
if __name__ == '__main__':
    MeticsSpatialDistributionShow3(lead_time=3)