...
# 拟合预测SST的LSTM模型
def fit_LSTM_model(X_train, y_train, n_neurons, n_inputs, dropout_rate, n_outputs, epochs, batch_size):
    model = Sequential()
    if n_layers == 1:
        model.add(LSTM(n_neurons, activation='relu', kernel_initializer='glorot_uniform',
                       input_shape=(n_steps, n_inputs)))  # 一层隐藏层
        model.add(Dropout(dropout_rate))
    else:
        model.add(LSTM(n_neurons, activation='relu', return_sequences=True,
                       kernel_initializer='glorot_uniform', input_shape=(n_steps, n_inputs)))  # 一层隐藏层
        model.add(Dropout(dropout_rate))
    if n_layers > 1:
        ...
        model.add(LSTM(n_neurons, activation='relu',
                       kernel_initializer='glorot_uniform'))
        model.add(Dropout(dropout_rate))
    # model.add(Dense(n_neurons, kernel_initializer='glorot_uniform'))
    ...
    temp = curtime + '_LSTM_best_model.h5'
...
if path.exists('sub-models_LSTM'):
    rmtree('sub-models_LSTM')
makedirs('sub-models_LSTM')

n_members = 100
rmse_test = []
for i in range(n_members):
    model = fit_LSTM_model(X_train, y_train, n_neurons, n_inputs, dropout_rate, n_outputs, epochs, batch_size)
    filename = 'sub-models_LSTM/model_LSTM_' + str(i + 1) + '.h5'
    model.save(filename)
...
dt.to_csv('LSTM_rmse_test.csv', index=False, header=False)
