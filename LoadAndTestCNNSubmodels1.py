...
# 拟合预测SST的CNN模型
def fit_CNN_model(X_train, y_train, filters, kernel_size, n_steps, n_inputs, n_layers, dropout_rate, n_outputs, epochs, batch_size):
    model = Sequential()
    # 1
    ...
    temp = curtime + '_CNN_best_model.h5'
...
if path.exists('sub-models_CNN'):
    rmtree('sub-models_CNN')
makedirs('sub-models_CNN')

n_members = 100
rmse_test = []
for i in range(n_members):
    model = fit_CNN_model(X_train, y_train, filters, kernel_size, n_steps, n_inputs, n_layers, dropout_rate, n_outputs, epochs, batch_size)
    filename = 'sub-models_CNN/model_CNN_' + str(i + 1) + '.h5'
    model.save(filename)
...
dt.to_csv('CNN_rmse_test.csv', index=False, header=False)
